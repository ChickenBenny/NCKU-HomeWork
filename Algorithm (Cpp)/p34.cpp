...
int escape(vector<vector<int>> &arr, vector<vector<bool>> &visit, int n, int m, point start, point end);
...
int main(){
    ...
    cout << escape(arr, visit, n, m, start, end);
}
...
int escape(vector<vector<int>> &arr, vector<vector<bool>> &visit, int n, int m, point start, point end){
    ...
    while(!path.empty()){
        ...
        if(current.x + 1 < n && arr[current.x + 1][current.y] == 1){
            next.x = current.x + 1, next.y = current.y, next.dist = current.dist + 1;
        ...
        if(current.y + 1 < m && arr[current.x][current.y + 1] == 1){
            next.x = current.x, next.y = current.y + 1, next.dist = current.dist + 1;
            if(visit[next.x][next.y] == false){
                path.push(next);
                visit[next.x][next.y] = true;
            }
