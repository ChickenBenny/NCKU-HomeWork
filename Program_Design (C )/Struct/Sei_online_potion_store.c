...
void sum_total_costs(order_t orders[], unsigned order_cnt)
{
    for (int i = 0;i < (int)order_cnt;i ++){
        for (int j = 0;j < (int) orders[i].item_cnt;j ++){
            orders[i].total_cost += orders[i].items[j].cnt * orders[i].items[j].cost;
        }
    }
}
...
void reorder(order_t orders[], unsigned order_cnt)
{
    qsort(orders, (int)order_cnt, sizeof(orders[0]), cmp);
}

int cmp(const void *a, const void *b){
    order_t *c = (order_t *)a;
    order_t *d = (order_t *)b;
    if (c->pick_up_time.hour < d->pick_up_time.hour)
        return -1;
    else if ((c->pick_up_time.hour == d->pick_up_time.hour) && (c->pick_up_time.minute < d->pick_up_time.minute))
        return -1;
    else
        return 1;
}